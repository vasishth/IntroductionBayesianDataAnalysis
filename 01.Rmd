...
- The ideas and concepts in this lecture are often not taught in statistics courses in linguistics and psychology (I never was).
- The usual kind of cookbook approach to teaching statistics leads to all kinds of misunderstandings that
  have a snowball effect and are a big part of the cause for the replication crisis and all the chaos we see so often in empirical work in linguistics and psychology.  

It only takes about a day to understand these materials, but understanding the content here will positively 
...
**2. Compute probabilities of particular events (0,1,\dots,10 successes when n=10): dbinom**
...
means that the random variable $X$ has PDF $f(\cdot)$. 
...
We can **truncate** the normal distribution such that $S_X$ is bounded between some lower bound and upper bound--this comes later.
...
- The variance: $Var(Y) = \sigma^2$
  - Estimated by $\hat\sigma^2  = s^2 = \frac{\sum (y_i-\bar{y})^2}{n-1}$
