...
- $x\geq 0$ (a vector): the observed numbers of regressions per word are independent given $\lambda$
...
- First, note that the gamma distribution in general is $Gamma(a,b) \propto \lambda^{a-1} \exp(-\lambda b)$. 
...
\begin{equation}
k \int_{0}^{\infty} \lambda^{a^{*}-1} \exp(-\lambda b^{*})=1
\end{equation}
